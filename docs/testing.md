...
matrix also changes representative parser and preview-only settings, including
the emoji master/shortcut dependency, waits for the real Markdown plugin reload,
verifies disabled output, then resets the settings and verifies default
rendering returns.
